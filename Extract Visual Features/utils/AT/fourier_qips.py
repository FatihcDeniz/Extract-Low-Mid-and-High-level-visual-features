...
def rotavg_fast(array):
    ...
    rho = np.round(np.hypot(X, Y)).astype(np.int32)

    theta = np.arctan2(Y, X)

    max_r = N // 2

    f = np.zeros((max_r + 1, 9), dtype=np.float64)
    ...
    # column 8: all angles
    sums = np.bincount(rho_flat, weights=values, minlength=max_r + 1)
    counts = np.bincount(rho_flat, minlength=max_r + 1)

    nz = counts > 0
    f[nz, 8] = sums[nz] / counts[nz]
    ...
    theta_flat = theta_folded[mask].ravel()

    bins = np.digitize(theta_flat, edges) - 1
    ...
    for k in range(8):
        ...
        sums = np.bincount(r, weights=v, minlength=max_r + 1)
        counts = np.bincount(r, minlength=max_r + 1)

        nz = counts > 0
        f[nz, k] = sums[nz] / counts[nz]
...
def fourier_redies_vectorized(img_gray, bin_size=2, cycles_min=10, cycles_max=256):
    '''
    Calculates the 'Fourier Slope **Redies** and the Fourier Sigma' QIPs 
